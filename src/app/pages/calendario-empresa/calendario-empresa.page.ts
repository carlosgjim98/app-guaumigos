...
@Component({
  ...
  templateUrl: './calendario-empresa.page.html',
  styleUrls: ['./calendario-empresa.page.scss'],
})
export class CalendarioEmpresaPage implements OnInit {
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      this.allEvents.push({ id: index, title: 'Limado de uñas con Manchitas', date: formattedDate , time: formattedTime, color: color, class: 'color2' });
      index++;
      ...
      if(Math.random() < 0.5) {
        ...
        let formattedTime = `${String(randomHours).padStart(2, '0')}:${String(randomMinutes).padStart(2, '0')}:${String(randomSeconds).padStart(2, '0')}`;
        let color = `#${Math.floor(Math.random()*16777215).toString(16)}`;
        this.allEvents.push({ id: index, title: 'Corte de pelo a tu mascota', date: formattedDate , time: formattedTime, color: color , class: 'color2'});
        index++;
      }
      ...
      if(Math.random() < 0.5) {
        ...
        let formattedTime = `${String(randomHours).padStart(2, '0')}:${String(randomMinutes).padStart(2, '0')}:${String(randomSeconds).padStart(2, '0')}`;
        let color = `#${Math.floor(Math.random()*16777215).toString(16)}`;
        this.allEvents.push({ id: index, title: 'Veterinario', date: formattedDate , time: formattedTime, color: color ,class: 'color1'});
        index++;
      }
    }
